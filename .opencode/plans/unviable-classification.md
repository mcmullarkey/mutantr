# Feature: Unviable Outcome Classification for mutantr

## Goal
When a mutant causes an error during package loading (`source()` fails), classify the outcome as `"unviable"` instead of `"caught"`, exclude it from the mutation score denominator, and surface it in all output formats (data.frame, JSON, Markdown, console). This fixes a metric correctness bug where load errors inflate the mutation score.

## Dependencies
- AC1 has no dependencies — it is a standalone leaf change

## Acceptance Criteria

### AC1: Distinguish unviable (source/load error) from caught (test failure) in mutation test runner
- **Depends on:** none
- **Risk:** low
...
## Open Questions
None — all speculator disagreements were resolved by the resolver without needing user clarification.
